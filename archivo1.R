...
pie(sub_tabla$x,labels = sub_tabla$x,clockwise = TRUE,col = rainbow(length(sub_tabla$x)),cex=0.8, main =  "TOTAL DE VENTAS DE ROPA EN PAISES DE CENTRO AMERICA")
legend("left",legend=sub_tabla$Paises_CA,bty="n",fill=rainbow(length(sub_tabla$x)),cex = 0.8)
...
barplot(sub_tabla$x, main="Numero de Articulos de las ventas Offline vs Online",
        xlab="Canales de distribucion", col=c("darkblue","red"),
        names.arg= sub_tabla$x,xlim = c (0, 3))

legend("right",legend=sub_tabla$canales,bty="n",fill=c("darkblue","red"),cex = 0.8)
...
mybar<-barplot(sub_tabla$x, main="Años con mas ordenes de prioridad M",
        xlab="Años",ylab="Ordenes", col=rainbow(length(sub_tabla$x)),
        names.arg= paste(sub_tabla$anios,sub_tabla$x, sep="\n"),xlim = c (0, 15),ylim = c (0, 200),legend = sub_tabla$anios)
